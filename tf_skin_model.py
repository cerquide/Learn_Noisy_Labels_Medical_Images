...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        dec4 = self.dec4(torch.cat([up4, F.interpolate(enc4, scale_factor=2, mode='nearest', align_corners=True)], 1))
        up3 = self.upconv3(dec4)
        dec3 = self.dec3(torch.cat([up3, F.interpolate(enc3, scale_factor=2, mode='nearest', align_corners=True)], 1))
        up2 = self.upconv2(dec3)
        dec2 = self.dec2(torch.cat([up2, F.interpolate(enc2, scale_factor=2, mode='nearest', align_corners=True)], 1))
        up1 = self.upconv1(dec2)
        dec1 = self.dec1(torch.cat([up1, F.interpolate(enc1, scale_factor=2, mode='nearest', align_corners=True)], 1))
